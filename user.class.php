...
if (!defined('SQLITE_FILE'))
    define('SQLITE_FILE', './test.sdb'); // SQLite Filename and path
...
class userAuth extends PDO {
    ...
    public function user_login($username, $password) {
        ...
        if (!$result) {
            if (DEBUG)
                trigger_error('User `' . $username . '` does not exist!', E_USER_WARNING);
            $this->_destroy();
            return false;
        }
        ...
        if (!$this->validate_password(trim($password), $result)) {
            if (DEBUG)
                trigger_error('Password for user `' . $username . '` did not validate!', E_USER_WARNING);
            $this->_destroy();
            return false;
        }
