        ...
        <div class="container">
            <div class="row">
                ...
                if ($result = $conn->query($query)) {
                    ...
                    $r = 0;
                    if ($r < $rowcount) {
                        $remp = str_replace("_", " ", $tble);
                        echo '<div class="col-md-12"><h2>The ' . $remp . ' table is added for query builder</h2></div>' . "\n";
                        echo'<div class="form-group">                          
  <div class="col-md-4">
   <a type="button" id="editrow" name="editrow" class="btn btn-primary" href="querybuilder.php?w=editor&tbl=' . $tble . '"><span class="glyphicon glyphicon-plus"></span> Go to query editor</a>
  </div>
</div>' . "\n";
                        if (file_exists($vfile)) {
                            unlink($vfile);
                        ...
                        $dq = '$query="INSERT INTO table_queries (name_table, col_name) VALUES';
                        $addq = array();
                        while ($finfo = $qresult->fetch_field()) {
                            if ($finfo->name == $ncol) {
                                continue;
                            } else {
                                $addq[] = "('" . $tble . "', '" . $finfo->name . "')";
                            }
                        }
                        $dq .= implode(", ", $addq);
                        $dq .= '";';
  ...
  <div class="col-md-4">
    <button id="addtable" name="addtable" class="btn btn-info" href="http://' . $host . $uri . $redir . '">Add table</button>
        ...
        <div class="container-fluid">
            <center><h2 class="text-primary">Add query for column</h2></center>
            <?php
            $tble = protect($_GET['tbl']);
            $sql1 = "SHOW COLUMNS FROM " . $tble;
            $result1 = $conn->query($sql1);
            ...
            while ($finfo = $qresult->fetch_assoc()) {
                ...
                echo '<div class="form-group">
                            <label class="col-md-4 control-label" for="type">Select Type for ' . ucfirst($remp) . '</label>
                            <div class="col-md-4">
            ...
            echo '<div class="form-group">
                        <button type="submit" id="updatequeries" name="updatequeries" class="btn btn-primary"><span class="glyphicon glyphicon-plus"></span> Add queries</button>
                      </div>' . "\n";
            echo '</form>' . "\n";
            ...
            $scpt = $c->addPost($tble, $ncol);
            $ncols = $c->addTtl($tble, $ncol);
            $nvals = $c->addTPost($tble, $ncol);
            $mpty = $c->ifMpty($tble, $ncol);
            ...
            if (isset($_POST['submitrv'])) {
                ...
                $sqli = "UPDATE table_queries SET type='$sltb', joins='$joins', j_table='$tb', j_id='$sr', j_value='$sv' WHERE tque_Id='$inp'";
                $conn->query($sqli);
        ...
        <div class="form-group">  
            <div id="seltables" name="seltables">
            </div>
        </div>
        <div class="form-group">  
            <div class="col-md-12">
